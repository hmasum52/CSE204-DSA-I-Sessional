    ...
    for (int d = 1; d <= dices; d++) //d: number of dices
    {
        ...
        for (int s = d; s <= sum; s++) //s: sum 's' with 'd' dices
        {
            ...
            if(s <= faces[d-1]){
                ...
                dp[d][s] = (dp[d-1][s-1]%mod + dp[d][s-1]%mod- dp[d-1][s-faces[d-1]-1]%mod)%mod;
            }
            ...
            // again dp[3][7] = dp[2][6] + dp[2][5] + dp[2][4]
            // so dp[3][8] = dp[2][7] + dp[3][7] - dp[2][4]
            // we remove additional dp[2][4] here.
            if(s > faces[d-1])
                dp[d][s] -= dp[d-1][s-faces[d-1]-1]%mod; */
...
/*
g++ main.cpp
a.exe<"test-case/hisam/Test-6.txt"
